...
module.exports = async (role) => {
    try {
        ...
        await ch.send(roleCreateEmbed({ roleName: name, roleId: id, timeStr }))
            .catch(err => auditLogDebug('roleCreate', 'fallo al enviar:', err?.message || err));
    } catch (err) {
        auditLogDebug('roleCreate', 'error inesperado:', err?.message || err);
    }
};
