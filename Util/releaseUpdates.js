...
function estimateChangeMagnitude(latest, previous, pendingCommitSubjects = []) {
    ...
    for (const b of [...(latest?.bullets || []), ...(pendingCommitSubjects || [])]) {
...
function getReleaseContext({ lastAnnouncedVersion = '', lastAnnouncedCommit = '' } = {}) {
    ...
    const magnitude = estimateChangeMagnitude(current, previous, pendingCommitSubjects);
...
function gitRefExists(ref) {
    ...
    const resolved = readGitText(['rev-parse', '--verify', cleanRef]);
...
async function buildAiSummaryLines(ctx) {
    ...
    const user = [
        ...
        pendingCommitSubjects.length ? pendingCommitSubjects.map((c) => `- ${c}`).join('\n') : '- Sin commits pendientes',
        '',
        'Commits reales entre versiones:',
        effectiveCommitSubjects.length ? effectiveCommitSubjects.map((c) => `- ${c}`).join('\n') : '- Sin commits disponibles',
        '',
...
async function announceUpdateForGuild(client, guild, { force = false } = {}) {
    ...
    const ctx = getReleaseContext({
        lastAnnouncedVersion: lastAnnounced,
        lastAnnouncedCommit,
    });
    if (!force && lastAnnounced === ctx.currentVersion && !ctx.pendingCommitSubjects.length) {
        return { ok: false, reason: 'already-announced' };
    }

    const hasVersionChange = lastAnnounced !== ctx.currentVersion;
