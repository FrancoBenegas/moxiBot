...
function getReleaseByVersion(version) {
    ...
    return ordered.find((release) => release.version === String(version || '')) || null;
}
...
function readGitText(args) {
    try {
        return String(execFileSync('git', ['-C', getRepoRoot(), ...args], {
            encoding: 'utf8',
...
function gitRefExists(ref) {
    ...
    return Boolean(readGitText(['rev-parse', '--verify', cleanRef]));
}

function getHeadCommit() {
    return readGitText(['rev-parse', 'HEAD']);
}

function normalizeCommitSubject(line) {
    return String(line || '')
        .replace(/^[a-f0-9]+\s+/i, '')
        .replace(/^feat:\s*/i, '')
        .replace(/^fix:\s*/i, '')
...
function readCommitSubjectsFromRange(range, limit = 30) {
    const stdout = readGitText(['log', '--oneline', '-n', String(limit), range]);
    if (!stdout) return [];

    return stdout
        .split(/\r?\n/)
        .map((line) => normalizeCommitSubject(line))
        .filter(Boolean);
}
...
function readCommitSubjectsBetweenTags(previousVersion, currentVersion, limit = 30) {
    ...
    return readCommitSubjectsFromRange(range, limit);
}

function readCommitSubjectsBetweenRefs(fromRef, toRef = 'HEAD', limit = 30) {
    const from = String(fromRef || '').trim();
    const to = String(toRef || 'HEAD').trim() || 'HEAD';
    const range = from ? `${from}..${to}` : to;
    return readCommitSubjectsFromRange(range, limit);
}

function getPendingCommitSubjects({ currentVersion, lastAnnouncedVersion = '', lastAnnouncedCommit = '', limit = 20 } = {}) {
    const announcedCommit = String(lastAnnouncedCommit || '').trim();
    const announcedVersion = String(lastAnnouncedVersion || '').trim();
    const versionTag = getTagName(currentVersion);

    if (announcedCommit && gitRefExists(announcedCommit)) {
        return readCommitSubjectsBetweenRefs(announcedCommit, 'HEAD', limit);
    }

    if (announcedVersion && announcedVersion === String(currentVersion || '') && gitRefExists(versionTag)) {
        return readCommitSubjectsBetweenRefs(versionTag, 'HEAD', limit);
    }

    return [];
}

function estimateChangeMagnitude(latest, previous, pendingCommitSubjects = []) {
    if (!latest && !pendingCommitSubjects.length) return { level: 'unknown', score: 0, label: 'Sin datos' };
    ...
    const pendingCommits = Array.isArray(pendingCommitSubjects) ? pendingCommitSubjects.length : 0;

    let score = latestBullets + diffBullets + pendingCommits;
    const keywords = ['sistema', 'modulo', 'integracion', 'panel', 'streaming', 'music', 'perfil', 'comando'];

    for (const value of [...(latest?.bullets || []), ...(pendingCommitSubjects || [])]) {
        const low = String(value || '').toLowerCase();
        if (keywords.some((keyword) => low.includes(keyword))) score += 1;
    }
...
function getReleaseContext({ lastAnnouncedVersion = '', lastAnnouncedCommit = '' } = {}) {
    ...
    let previous = null;
    if (current) {
        const idx = ordered.findIndex((release) => release.version === current.version);
        if (idx > 0) previous = ordered[idx - 1];
    }

    const headCommit = getHeadCommit();
    const pendingCommitSubjects = getPendingCommitSubjects({
        currentVersion,
    ...
    return {
        ...
        current,
        previous,
        magnitude: estimateChangeMagnitude(current, previous, pendingCommitSubjects),
        headCommit,
...
function buildHeuristicSummaryLines(ctx) {
    const pendingCommits = (ctx?.pendingCommitSubjects || []).slice(0, 6);
    const releaseBullets = (ctx?.current?.bullets || []).slice(0, 6);
    const commitBullets = readCommitSubjectsBetweenTags(ctx?.previous?.version, ctx?.currentVersion, 12).slice(0, 6);

    const bullets = pendingCommits.length ? pendingCommits : (releaseBullets.length ? releaseBullets : commitBullets);
    if (!bullets.length) {
...
async function buildAiSummaryLines(ctx) {
    ...
    const pendingCommitSubjects = (ctx.pendingCommitSubjects || []).slice(0, 20);
    const effectiveCommitSubjects = pendingCommitSubjects.length ? pendingCommitSubjects : commitSubjects;

    if (!currentBullets.length && !effectiveCommitSubjects.length) return [];
    ...
    const user = [
        ...
        '',
        'Commits nuevos sin anunciar:',
        pendingCommitSubjects.length ? pendingCommitSubjects.map((value) => `- ${value}`).join('\n') : '- Sin commits pendientes',
        '',
        'Commits reales entre versiones:',
        effectiveCommitSubjects.length ? effectiveCommitSubjects.map((value) => `- ${value}`).join('\n') : '- Sin commits disponibles',
        '',
        'Cambios reportados en release actual:',
        currentBullets.length ? currentBullets.map((value) => `- ${value}`).join('\n') : '- Sin bullets',
        '',
        'Cambios reportados en release anterior:',
        previousBullets.length ? previousBullets.map((value) => `- ${value}`).join('\n') : '- Sin bullets',
    ].join('\n');
    ...
    return validateAiSummaryLines(lines, [...currentBullets, ...effectiveCommitSubjects]);
}

async function buildVersionPanel({ guildName = '', preferAi = true, lastAnnouncedVersion = '', lastAnnouncedCommit = '' } = {}) {
    const ctx = getReleaseContext({ lastAnnouncedVersion, lastAnnouncedCommit });
    let aiLines = [];
    if (preferAi) {
    ...
    const body = [
        ...
        `Commits nuevos detectados: **${ctx.pendingCommitSubjects.length}**`,
...
async function announceUpdateForGuild(client, guild, { force = false } = {}) {
    ...
    const lastAnnouncedVersion = String(settings?.UpdateLastAnnouncedVersion || '').trim();
    const lastAnnouncedCommit = String(settings?.UpdateLastAnnouncedCommit || '').trim();

    if (!channelId) return { ok: false, reason: 'no-channel' };
    if (!force && !autoEnabled) return { ok: false, reason: 'auto-disabled' };

    const ctx = getReleaseContext({ lastAnnouncedVersion, lastAnnouncedCommit });
    if (!force && lastAnnouncedVersion === ctx.currentVersion && !ctx.pendingCommitSubjects.length) {
        return { ok: false, reason: 'already-announced' };
    }

    const hasVersionChange = lastAnnouncedVersion !== ctx.currentVersion;
    const hasPendingCommitChanges = ctx.pendingCommitSubjects.length > 0;
    const shouldAnnounce = force || (
        (hasVersionChange && ctx.magnitude.level === 'large')
        || (hasPendingCommitChanges && ctx.magnitude.level !== 'small')
    );
    ...
    const payload = await buildVersionPanel({
    ...
    if (ctx.headCommit) {
        await setGuildUpdatesLastAnnouncedCommit(guild.id, ctx.headCommit).catch(() => null);
    }
