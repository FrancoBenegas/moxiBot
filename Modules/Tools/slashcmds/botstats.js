...
module.exports = {
    ...
    async run(Moxi, interaction) {
        ...
        debugHelper.log('botstats', 'slash start', { guildId, requesterId, publicReply });
        ...
        const interactionLatency = Number.isFinite(interaction.createdTimestamp)
            ? `${Math.max(0, Date.now() - interaction.createdTimestamp)}ms`
            : 'n/a';
        ...
        const discordBlock = kvBlock(t('BOTSTATS_DISCORD', 'Discord'), [
            ...
            [t('BOTSTATS_LATENCY', 'Latencia'), interactionLatency],
