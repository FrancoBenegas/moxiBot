...
function isOn(v) {
    return ['on', 'enable', 'enabled', 'si', 'true', '1'].includes(String(v || '').toLowerCase());
}

function isOff(v) {
    return ['off', 'disable', 'disabled', 'no', 'false', '0'].includes(String(v || '').toLowerCase());
}
...
module.exports = {
    ...
    async execute(Moxi, message, args) {
        ...
        const versionPanel = await buildVersionPanel({ guildName: message.guild?.name || '' });
